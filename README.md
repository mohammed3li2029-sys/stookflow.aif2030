...
- **الإشعارات** — لا تحتاج ربط أصلًا؛ هذي الشاشة تُحسب تلقائيًا من بيانات
  ...
  كود إضافي.
- **المشاريع** (`projects`) — الإنشاء والتعديل الأساسي للمشروع نفسه مربوط بالكامل.
  **استثناء:** التعديلات الدقيقة داخل مشروع موجود (إضافة صورة، ملاحظة، مهمة فرعية،
  عضو فريق، مستند...) لسه ما تُزامن تلقائيًا بسبب التعقيد الشديد لبنية بيانات
  المشروع المتداخلة — ممكن أربطها بجولة لاحقة لو احتجتها.

الوحدة التالية لا تزال بيانات تجريبية محلية بالكامل (تفاصيل فرعية معقدة، خارج
نطاق هذي الجولة): تعديلات دقيقة داخل مشروع موجود (صور، ملاحظات، مهام فرعية).
